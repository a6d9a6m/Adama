...
import (
	...
	"github.com/littleSand/adama/app/ops-ai/internal/server"
...
func main() {
	...
	repo, cleanup, err := data.NewData(&bc.Data, logger)
	if err != nil {
		panic(err)
	}
	defer cleanup()

	uc := biz.NewOpsAIUsecase(repo, logger)
	svc := service.NewOpsAIService(uc, logger)
	httpServer := server.NewHTTPServer(&bc.Server, svc, logger)
